<template>
    <div>
        ...
        <div v-else>
            <BasePicker v-if="editMode" searchApiPath="authors" searchParameterName="email"  idField="id" nameField="email" path="authors" label="AuthorId" v-model="value" @selected="pick" :editMode="editMode" />
            <div v-else style="height:100%">
                <span>{{ value && value.email ? value.email : '' }}</span>
            </div>
        </div>
    </div>
